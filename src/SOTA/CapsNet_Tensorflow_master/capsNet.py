...
from src.SOTA.CapsNet_Tensorflow_master.config import cfg
from src.SOTA.CapsNet_Tensorflow_master.utils import get_batch_data
from src.SOTA.CapsNet_Tensorflow_master.utils import softmax
from src.SOTA.CapsNet_Tensorflow_master.utils import reduce_sum
from src.SOTA.CapsNet_Tensorflow_master.capsLayer import CapsLayer
...
class CapsNet(object):
    def __init__(self, is_training=True, height=28, width=28, channels=1, num_label=10):
        ...
        with self.graph.as_default():
            if is_training:
                ...
                self.X = tf.compat.v1.placeholder(tf.float32, shape=(cfg.batch_size, self.height, self.width, self.channels))
                self.labels = tf.compat.v1.placeholder(tf.int32, shape=(cfg.batch_size, ))
    ...
    def build_arch(self):
        with tf.compat.v1.variable_scope('Conv1_layer'):
            ...
            conv1 = tf.compat.v1.layers.conv2d(self.X, filters=256,
        ...
        with tf.compat.v1.variable_scope('Masking'):
            ...
            self.argmax_idx = tf.compat.v1.to_int32(tf.argmax(self.softmax_v, axis=1))
        ...
        with tf.compat.v1.variable_scope('Decoder'):
            ...
            fc1 = tf.compat.v1.layers.dense(vector_j, units=512)
            fc2 = tf.compat.v1.layers.dense(fc1, units=1024)
            self.decoded = tf.compat.v1.layers.dense(fc2, units=self.height * self.width * self.channels, activation=tf.sigmoid)
    ...
    def _summary(self):
        ...
        correct_prediction = tf.equal(tf.compat.v1.to_int32(self.labels), self.argmax_idx)
