...
def make_plot_accuracy():
    ...
    for i in range(0, len(y)):
        for j in range(0, len(y[i])):
            if j-n < 0:
                numbers = [v for v in y[i][0:j+n+1]]
                y[i][j] = sum(numbers)/(float(len(numbers)))
            elif j+n >= len(y[i]):
                numbers = [v for v in y[i][j-n:len(y[i])]]
                y[i][j] = sum(numbers)/(float(len(numbers)))
            else:
                numbers = [v for v in y[i][j-n:j+n+1]]
                y[i][j] = sum(numbers)/(float(len(numbers)))
...
def make_plot_loss():
    ...
    plt.savefig(fname=("moving_plot_los.svg"))
...
def make_plot_parameters():
    ...
    plt.savefig(fname=("moving_plot_par.svg"))
